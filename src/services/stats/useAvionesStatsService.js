...
export const useAvionesStatsService = {
  getAvionesStats: async () => {
    const query = `
            WITH aviones_total AS (SELECT COUNT(*) as total_aviones
                                   FROM avion_creado),
                 aviones_modelo AS (SELECT mac.mda_nombre   as modelo,
                                    ...
                                    FROM modelo_avion_conf mac
                                             ...
                                             LEFT JOIN detalle_sld_cliente dsc ON mac.mda_id = dsc.fk_mda_id
                                             LEFT JOIN solicitud_cliente sc ON dsc.fk_sct_id = sc.sct_id
                                             LEFT JOIN avion_creado ac ON sc.sct_id = ac.fk_sct_id
                                    GROUP BY mac.mda_id, mac.mda_nombre, ta.tiv_nombre),
                 aviones_proceso AS (SELECT COUNT(*) as aviones_en_proceso
                                     FROM fase_ensamble_avion
                                     WHERE fln_fecha_fin IS NULL),
                 aviones_mes AS (SELECT COUNT(*) as aviones_nuevos
                                 FROM avion_creado
                                 WHERE avi_fecha_creacion >= CURRENT_DATE - INTERVAL '30 days'),
                 pruebas_stats AS (SELECT COUNT(*) FILTER (WHERE pzb_fecha_fin IS NULL)                         as pendientes,
                                          COUNT(*) FILTER (WHERE pzb_fecha_fin IS NOT NULL)                     as completadas,
                                          COUNT(*)
                                          FILTER (WHERE pzb_fecha_inicio IS NOT NULL AND pzb_fecha_fin IS NULL) as en_proceso
                                   FROM prueba_pieza_solicitud)
            SELECT t.total_aviones                                           as total,
